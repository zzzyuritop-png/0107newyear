...
const PinkTreeParticles: React.FC<{ explosionValue: number }> = ({ explosionValue }) => {
  ...
  useFrame((state, delta) => {
    if (materialRef.current) {
      ...
      currentExplosion.current = THREE.MathUtils.lerp(currentExplosion.current, explosionValue, delta * 3.0);
  ...
  return (
    <points ref={pointsRef}>
      ...
      <shaderMaterial
        ref={materialRef}
        uniforms={{ uTime: { value: 0 }, uHeight: { value: CONFIG.treeHeight }, uExplosion: { value: 0 }}}
        vertexShader={`
          uniform float uTime; uniform float uHeight; uniform float uExplosion; 
          attribute float aSize; attribute float aRandomness; attribute vec3 aColor; attribute vec3 aExplosionDir;
          varying vec3 vColor; varying float vAlpha;
          void main() {
            vColor = aColor; vec3 pos = position;
            float breath = sin(uTime * 1.5 + pos.y * 0.5 + aRandomness * 5.0);
            pos.x += pos.x * (breath * 0.02); pos.z += pos.z * (breath * 0.02);
            ...
            explosionOffset.x = nx; explosionOffset.z = nz;
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            float glow = 1.0 - (ll * 2.0); glow = pow(glow, 2.0); 
