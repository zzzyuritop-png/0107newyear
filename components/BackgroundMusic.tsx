...
const MUSIC_URL = "https://music.163.com/song/media/outer/url?id=1895244308.mp3";
...
const BackgroundMusic: React.FC = () => {
  ...
  return (
    <div className="absolute top-4 right-4 z-50 flex flex-col items-end gap-2 pointer-events-auto">
      ...
      <button 
        onClick={togglePlay} 
        className={`group flex items-center justify-center w-10 h-10 rounded-full border border-white/20 backdrop-blur-md transition-all duration-500 ${isPlaying ? 'bg-white/10' : 'bg-transparent'}`}
      >
        {isPlaying ? (
      ...
      <div className={`text-[10px] text-white/50 font-serif tracking-widest uppercase transition-opacity duration-700 ${isPlaying ? 'opacity-100' : 'opacity-0'}`}>
        赶路的蝴蝶
      </div>
    </div>
  );
